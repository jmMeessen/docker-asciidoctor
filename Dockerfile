...
#RUN  curl -L -s https://api.github.com/repos/asciidoctor/asciidoctor-fopub/tarball | tar xzf - -C /fopub/ --strip-components=1
#  && touch empty.xml \
#  && fopub empty.xml \
#  && rm empty.xml
...
RUN gem install --no-ri --no-rdoc asciidoctor-pdf --version 1.5.0.alpha.10
RUN gem install --no-ri --no-rdoc asciidoctor-confluence
RUN gem install --no-ri --no-rdoc coderay pygments.rb thread_safe epubcheck kindlegen
...
# RUN curl -L -s https://bitbucket.org/pypa/setuptools/raw/bootstrap/ez_setup.py | python
